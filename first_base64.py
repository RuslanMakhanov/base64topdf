...
def log_message(message, log_type="access"):
    log_file = ACCESS_LOG_FILE if log_type == "access" else ERROR_LOG_FILE
                    ...
                    if unique_code == excel_code: # Сравниваем уникальный код
                        ...
                        if row[3]:
                            if "{{" in row[3] and "}" in row[3]:
                                try:
                                    file_name = row[3].split("{{")[1].split("}")[0]
                                    print(f"Имя файла: {file_name}")
                                    log_message(f"Имя файла: {file_name}")
                                except IndexError:
                                    print(f"Ошибка извлечения имени файла для УК {unique_code}: {row[3]}")
                                    log_message(f"Ошибка извлечения имени файла для УК {unique_code}: {row[3]}", log_type="error")
                        else:
